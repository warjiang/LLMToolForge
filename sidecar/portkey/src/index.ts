 ...
 */
app.use('*', (c, next) => {
  // LLMToolForge: 'bun' added so the compiled-with-bun sidecar skips compression
  // (avoids interfering with SSE streaming), matching the node runtime path.
  const runtimesThatDontNeedCompression = ['lagon', 'workerd', 'node', 'bun'];
